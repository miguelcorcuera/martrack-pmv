...
function DeliveryDetailPage() {
  ...
  const { data: delivery, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("*, vehicles(plate,brand,model), municipalities(name), supervisor:supervisor_employee_id(id,full_name,auth_user_id), recipient:recipient_employee_id(id,full_name)")
        .eq("id", id).single();
  ...
  const canSign = isAssignedSupervisor && !signature && !["firmado","cerrado","cancelado"].includes(delivery?.status);
  const canClose = (isRoot || isCoordinator) && delivery?.status === "firmado";
  ...
  const onSign = async () => {
    ...
    try {
      ...
      await supabase.from("vehicles").update({
        current_responsible_employee_id: delivery.recipient_employee_id,
        status: "asignado",
  ...
  const onClose = async () => {
    const { error } = await supabase.from("vehicle_deliveries").update({ status: "cerrado", closed_at: new Date().toISOString() }).eq("id", id);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <StatusBadge value={delivery.status} tone={statusTone(delivery.status)} />
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <dl className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><dt className="text-xs uppercase tracking-wider text-muted-foreground">Receptor</dt><dd>{delivery.recipient?.full_name ?? "—"}</dd></div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                {(isRoot || isCoordinator) && delivery.status !== "cerrado" && delivery.status !== "cancelado" &&
